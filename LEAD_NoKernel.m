function [Eval, y_predict] = LEAD_NoKernel(X_train, y_train, X_test, y_test, theta, alpha, gamma, numK)
%LEAD_NoKernel Implements the non-kernelized (linear) LEAD algorithm.
%
%   Inputs:
...
%   Outputs:
%       Eval: Struct with evaluation metrics (HS, EM, SEM).
...
%% Step 0: Initialization
    ...
    % Compute Label Correlation C and Laplacian Lc
    C = squareform(1 - pdist(V_train', 'cosine'));
    Lc = diag(sum(C, 2)) - C; % C is symmetric

    % Solve Sylvester equation for the linear mapping matrix W
    A = X_train' * X_train + gamma * eye(num_features);
    B = alpha * (Lc + Lc');
    F = X_train' * V_train;
    W = sylvester(A, B, F);

    %% Step 4: Prediction and Evaluation
    % Predict enriched labels for test data (Eq. 17)
    F_test_pred = X_test * W;

    % one-hot label decoding
    for ii=1:num_testing
        dim_begin = 1;
        for dd=1:num_dim
            y_test_one_dim = F_test_pred(ii,(dim_begin:dim_begin+num_per_dim(dd)-1));
